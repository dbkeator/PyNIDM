"""
NIDM URI constants for the LinkML-based PyNIDM.

This module is the prov-toolbox-free port of ``nidm.core.Constants``.
Every legacy ``QualifiedName(provNamespace(...), term)`` constant has
been replaced with an ``rdflib.URIRef`` (built via the ``Namespace[term]``
expansion against the bindings defined in
:mod:`nidm.linkml.core.namespaces`).

What was dropped
----------------
* ``NIDMDocument`` -- the prov.ProvDocument subclass.  Replaced by
  :class:`nidm.linkml.experiment.core.Core`.
* ``q_graph`` and all ``*_QNAME`` constants -- prov-toolbox internals
  for computing qnames.  Not needed in the rdflib world.
* ``PROVONE_*`` machinery (PROVONE_N_MAP, PROVONE_ATTRIBUTES,
  PROVONE_RECORD_ATTRIBUTES, PROVONE_ATTRIBUTE_QNAMES, ...) -- legacy
  ProvONE compatibility for NIDM-Statistics; will be revisited if/when
  the NIDM-Statistics port lands (task 10).  The individual
  ``PROVONE_*`` URIRef constants are kept (they're just URIs).
* ``nidm_experiment_terms`` list -- only used by the legacy JSON-LD
  context generator.  Will be revisited if the Utils.py port needs it.

Drop-in compatibility
---------------------
Imports of the form ``from nidm.linkml.core.constants import NIDM_PROJECT``
are drop-in replacements for the corresponding legacy imports from
``nidm.core.Constants``, with the caveat that the constants here are
:class:`rdflib.URIRef` instances (the legacy ones were
``prov.model.QualifiedName``).  Code that consumes them via the
rdflib API (``graph.add((s, p, o))``, SPARQL, etc.) works without
change; code that consumes them as prov-toolbox objects does not.
"""
...
# ---------------------------------------------------------------------------
# NIDM-Statistics / NIDM-Results constants (URI shortcuts using the OBO/NIDM
# fragment IDs the legacy code uses).  These are kept verbatim since
# rdflib's Namespace already returns URIRefs from bracket access.
# ---------------------------------------------------------------------------
...
# NIDM-Results (NIDM_0000XXX fragment IDs)
...
# ---------------------------------------------------------------------------
# NIDM-Experiment constants (the bulk of what NIDM-Experiment code uses)
# ---------------------------------------------------------------------------

NIDM_DATAELEMENT = NIDM["DataElement"]
NIDM_PROJECT = NIDM["Project"]
NIDM_PROJECT_IDENTIFIER = SIO["Identifier"]
NIDM_PROJECT_NAME = DCTYPES["title"]
NIDM_PROJECT_DESCRIPTION = DCT["description"]
NIDM_DESCRIPTION = DCT["description"]
NIDM_DEFINITION = DCT["description"]
NIDM_PROJECT_LICENSE = DCT["license"]
NIDM_PROJECT_URL = SIO["URL"]
NIDM_PROJECT_REFERENCES = DCAT["creator"]
# NOTE: legacy used QualifiedName(provNamespace("ncit", DCAT), "author") -- the
# "ncit" prefix was bound to the DCAT namespace, which is a known bug in the
# legacy code.  We preserve the actual URI (DCAT["author"]) for parity.
NIDM_AUTHOR = DCAT["author"]
NIDM_SESSION = NIDM["Session"]
NIDM_ACQUISITION_ACTIVITY = NIDM["Acquisition"]
NIDM_ACQUISITION_MODALITY = NIDM["hadAcquisitionModality"]
NIDM_ASSESSMENT_ACQUISITION = ONLI["instrument-based-assessment"]
NIDM_ACQUISITION_ENTITY = NIDM["AcquisitionObject"]
NIDM_PROJECT_SOURCE = DCTYPES["source"]
NIDM_HAD_NUMERICAL_VALUE = NIDM["hadNumericalValue"]
NIDM_BATH_SOLUTION = NIDM["BathSolution"]
NIDM_CELL_TYPE = NIDM["CellType"]
NIDM_CHANNEL_NUMBER = NIDM["ChannelNumber"]
NIDM_ELECTRODE_IMPEDANCE = NIDM["ElectrodeImpedance"]
NIDM_GROUP_LABEL = NIDM["GroupLabel"]
NIDM_HOLLOW_ELECTRODE_SOLUTION = NIDM["HollowElectrodeSolution"]
NIDM_HAD_IMAGE_CONTRACT_TYPE = NIDM["hadImageContractType"]
NIDM_HAD_IMAGE_USAGE_TYPE = NIDM["hadImageUsageType"]
NIDM_NUBMER_OF_CHANNELS = NIDM["NubmerOfChannels"]
NIDM_APPLIED_FILTER = NIDM["AppliedFilter"]
NIDM_SOLUTION_FLOW_SPEED = NIDM["SolutionFlowSpeed"]
NIDM_RECORDING_LOCATION = NIDM["RecordingLocation"]
NIDM_DEMOGRAPHICS_ENTITY = NIDM["DemographicsInstrument"]
NIDM_ASSESSMENT_USAGE_TYPE = NIDM["AssessmentUsageType"]
NIDM_ASSESSMENT_ENTITY = ONLI["assessment-instrument"]
...
NIDM_PARTICIPANT = SIO["Subject"]

# Demographics
NIDM_AGE = NCICB["Age"]
NIDM_GENDER = NDAR["gender"]
NIDM_SEX = PATO["PhenotypicSex"]
NIDM_HANDEDNESS = OBO["handedness"]
NIDM_ETHNICITY = SIO["ethnicity"]
NIDM_RACE = SIO["race"]
...
# MRI scan types
NIDM_IMAGE_CONTRAST_TYPE = NIDM["hadImageContrastType"]
NIDM_IMAGE_USAGE_TYPE = NIDM["hadImageUsageType"]
NIDM_PET = NIDM["PositronEmissionTomography"]
NIDM_MRI = NIDM["MagneticResonanceImaging"]
NIDM_MRI_ANATOMIC_SCAN = NIDM["Anatomical"]
NIDM_MRI_STRUCTURE_SCAN = NIDM["Structural"]
NIDM_MRI_FUNCTION_SCAN = NIDM["Functional"]
NIDM_MRI_DWI_SCAN = NIDM["DiffusionWeighted"]
NIDM_MRI_DWI_BVAL = NIDM["b-value"]
NIDM_MRI_DWI_BVEC = NIDM["b-vector"]
NIDM_MRI_FUNCTION_TASK = NIDM["Task"]
NIDM_MRI_T1 = NIDM["T1Weighted"]
NIDM_MRI_T2 = NIDM["T2Weighted"]
NIDM_MRI_T2_STAR = NIDM["T2StarWeighted"]
NIDM_MRI_DIFFUSION_TENSOR = NIDM["DiffusionTensor"]
NIDM_MRI_FLOW = NIDM["FlowWeighted"]
NIDM_MRI_BOLD_EVENTS = NIDM["StimulusResponseFile"]
NIDM_MRI_ASL = NIDM["ArterialSpinLabeling"]

# Crypto / DataLad / DOI / Funding
CRYPTO_SHA512 = CRYPTO["sha512"]
DATALAD_LOCATION = DATALAD["Location"]
NIDM_DOI = EDAM["data_1188"]
NIDM_FUNDING = OBO["IAO_0000623"]
NIDM_ACKNOWLEDGEMENTS = OBO["IAO_0000324"]
...
# ---------------------------------------------------------------------------
# SCR (SciCrunch) / NLX (NeuroLex) software and instrument constants
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Common isAbout URIs (plain strings -- they're used as string-typed values,
# not as predicates).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# REST API constants -- plain strings used as JSON keys
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# DataLad named-tuple compatibility shim (legacy ``DD`` namedtuple).
# Some legacy CSV-mapping code uses this; we preserve the shape but it's
# not tied to prov-toolbox.
# ---------------------------------------------------------------------------
